...
class Waveform
  ...
  def channel_peak(frames, channel=0)
    ...
    frames.each do |frame|
      next if frame.nil?
      frame = frame[channel] if frame.respond_to?(:[])
      frame = frame.abs
      peak = frame if frame > peak
    end
    peak
  end
  ...
  def channel_rms(frames, channel=0)
    frames = frames.map { |f| f.respond_to?(:[]) ? f[channel] : f }
    avg = frames.inject(0.0){ |sum, frame| sum += frame ? frame : 0 }/frames.size.to_f
    Math.sqrt(frames.inject(0.0){ |sum, frame| sum += frame ? (frame-avg)**2 : 0 }/frames.size.to_f)
  end
end
